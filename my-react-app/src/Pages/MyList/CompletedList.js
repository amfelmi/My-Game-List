...
import Gold_icon from "./logo-no-background.png"
...
const CompletedList = () => {
    ...
    const [podiumGames, setPodiumGames] = useState([]);
    ...
    //PODIUM LOGIC____________________________________________________________________________
    // retreive which games in podium
    const fetchPodiumGames = (uid) => {
        axios.get(`http://localhost:3001/get-podium-games?uid=${uid}`)
            .then(response => {
                setPodiumGames(response.data);
            })
            .catch(error => {
    ...
    function getPodiumGame(position) {
        const podiumGame = podiumGames.find(p => p.position === position);

        if (podiumGame) {
            // using game state, find the matching game using gid
            const gid = podiumGame.GID;
            const gameData = games.find(game => game.gid === gid);
            if (gameData) {
                return (
                    <div>
                        <div>
                            <img className='podium-image' src={gameData.image} alt={gameData.title} />
                            <p className='podium-title'>{gameData.title}</p>
                        </div>
                        <div className='podium-selector-wrapper'>
                            <select className='podium-selector' onChange={(event) => handlePodiumChange(event, position)}>
    ...
    function handlePodiumChange(event, position) {
        const selectedGid = event.target.value;

        // Check if a game already exists in the podium position
        const existingPodiumGame = podiumGames.find(p => p.position === position);

        if (existingPodiumGame) {
            const confirmChange = window.confirm("A game already exists in this podium position. Do you want to replace it?");
            if (!confirmChange) {
                return;
            }
        }

        // Update or insert the podium game
        axios.post('http://localhost:3001/update-podium-game', { uid, gid: selectedGid, position })
            .then(response => {
                console.log(response.data.message);
                // Refresh the podium games list
                fetchPodiumGames(uid);
            })
            .catch(error => {
            ...
            <div className='mylist-page'>
                <div className='podium-container'>
                    <div className='podium-wrapper'>
                        <div className='stand-second'>
                            {getPodiumGame(2)}
                        </div>
                    </div>

                    <div className='podium-wrapper'>
                        <img className='gold-podium' src={Gold_icon} />
